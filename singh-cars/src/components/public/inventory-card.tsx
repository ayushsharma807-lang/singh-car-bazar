...
export function InventoryCard({ listing }: { listing: Listing }) {
  ...
  return (
    <Link href={`/inventory/${listing.id}`} className="block">
      <div className="group cursor-pointer overflow-hidden rounded-3xl border border-gray-200 bg-white shadow-sm transition hover:-translate-y-0.5 hover:shadow-md">
        {featuredImage ? (
          ...
          <PlaceholderMedia
            ...
            className="rounded-none border-0 bg-gray-50"
          />
        )}
