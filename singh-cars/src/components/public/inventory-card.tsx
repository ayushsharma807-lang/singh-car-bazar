...
export function InventoryCard({ listing }: { listing: Listing }) {
  const featuredImage =
    listing.coverImageUrl ?? listing.images[0]?.imageUrl ?? "";

  return (
    <Link href={`/inventory/${listing.id}`} className="block">
      <div className="cursor-pointer overflow-hidden rounded-[28px] border border-slate-200 bg-white shadow-[0_20px_60px_rgba(15,23,42,0.08)] transition-all duration-200 hover:scale-[1.02]">
        {featuredImage ? (
        ...
        <div className="space-y-4 p-6">
          ...
          <div className="flex items-center justify-between gap-4">
            <p className="text-2xl font-bold text-[#2252e8]">
              {formatPrice(listing.price)}
            </p>
            <span className="pointer-events-none inline-flex rounded-full bg-[#2252e8] px-5 py-3 text-sm font-semibold uppercase tracking-[0.2em] text-white transition hover:bg-[#173bb0]">
              View Details
            </span>
          </div>
        </div>
